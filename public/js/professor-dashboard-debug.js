...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log('DEBUG: Auth check result:', currentUser ? `Logged in as ${currentUser.name} (${currentUser.email})` : 'No user found');

  if (!currentUser || currentUser.role !== 'professor') {
    console.error('DEBUG: Auth failed or invalid role:', currentUser);
    // Do not redirect immediately, let the user see the error in the debug console
  ...
  try {
    ...
    if (!isConnected) {
      console.error('CRITICAL: Backend connection failed or Unauthorized');
      alert('Cannot connect to server or Session Expired. Please check logs.');
    } else {
      console.log('DEBUG: Connection OK. Fetching data...');
      await loadAllData();
...
async function checkConnection() {
  try {
    ...
    if (testRes.status === 401) {
      console.error("DEBUG: checkConnection -> 401 UNAUTHORIZED");
      return false;
    }
...
async function renderMessages() {
  ...
  // Create chat layout - matching student dashboard structure
  messagesEl.innerHTML = `
      ...
      <div class="chat-main" id="chat-window">
        <div class="empty-state">
          <i class="ri-chat-smile-2-line" style="font-size: 3rem; color: var(--primary-color);"></i>
  ...
  // Add event delegation for conversation clicks
  ...
  if (conversationList) {
    conversationList.addEventListener('click', (e) => {
      ...
      if (conversationItem) {
        ...
        const convName = conversationItem.dataset.conversationName;
        if (convId) {
    ...
    try {
      ...
      if (data.success) {
        ...
        conversationList.innerHTML = filteredConversations.map(conv => {
          ...
          return `
            <div class="chat-item ${unreadCount > 0 ? 'unread' : ''}" data-conversation-id="${conv.conversationId || conv.id}" data-conversation-name="${name}">
  ...
  // Load chat history
  // Load chat history
  async function loadChatHistory(conversationId, conversationName) {
  ...
  async function sendMessage(conversationId) {
    ...
    try {
      ...
      if (data.success) {
        input.value = '';
        // Reload chat to see new message
        const chatNameEl = document.querySelector('.chat-name');
        const chatName = chatNameEl ? chatNameEl.textContent : 'Chat';
        loadChatHistory(conversationId, chatName);
      }
    } catch (error) {
  ...
  window.deleteSelectedMessages = function () {
    alert("Delete selected - implementation pending backend support");
  }
  ...
  window.toggleDetailsPane = function () {
    ...
    if (pane) {
      ...
      if (pane.classList.contains('open')) {
        pane.innerHTML = '<div style="padding:20px"><h3>Details</h3><p>User info...</p></div>';
      }
    }
  }
  ...
  window.openCreateGroupModal = function () {
    ...
    alert("Group creation coming soon for Professors.");
  }
